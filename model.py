...
class Decoder(tf.keras.Model):
    def __init__(self, units, vocab_size, embedding_dim, Tx, batch_size):
        ...
        self.attention_module = Attention(Tx)
    ...
    def call(self, X, a, h, c, coverage, context, max_oov, X_mask, use_coverage):
        ...
        context, alpha_weights = self.attention_module(a, h, coverage, X_mask, use_coverage)
...
class TextSummarizer:
    ...
    def train_step(self, X, y, y_teacher_force, X_batch_indeces_ext, max_oov):
        ...
        with tf.GradientTape() as tape:
            ...
            context, _ = self.decoder.attention_module(a, h, coverage, X_mask, use_coverage=False) # initial getting initial value of context vec
            
            for t in range(self.Ty): #decoder loop, iter for each output word
                
                vocab_dist, alpha_weights, context, h, c, p_gen = self.decoder(decoder_input, a, h, c, coverage, context, max_oov, X_mask, self.use_coverage) #attention_dist = alpha_weights
                ...
                #loss += self._loss_function(y_true, y_pred, y_true_lengths)
                ...
                if self.use_coverage: # adding coverage loss = lambda * sum(min(alpha, coverage))
    ...
    def fit(self, epochs, train_data, val_data=None, early_stopping_patience=None):
        ...
        for epoch in range(epochs):
            ...
            for batch_i in range(0, total_train_batches):
                ...
                print_status_bar(epoch, "train", batch_i, total_train_batches, loss, accuracy, top_k_accuracy, t0)
            ...
            if val_data is not None: #validation
                ...
                for batch_i in range(0, total_val_batches):
                    ...
                    X_batch, X_batch_indeces_ext, y_batch_teacher_force, y_batch, max_oov = self._get_batch(batch_i, val_data)
                                        
                    loss, accuracy, top_k_accuracy = self.evaluate(X_batch, y_batch, X_batch_indeces_ext,  max_oov)
                    val_epoch_loss += loss
                    ...
                    print_status_bar(epoch, "val", batch_i, total_val_batches, loss, accuracy, top_k_accuracy, t0)

            # summary for entire epoch
            print(f"\r\nepoch: {epoch+1} DONE\
                \tavg_train_loss: {train_epoch_loss / total_train_batches}\
                \texec_time: {(time.time() - t0)/60:.2f}min", end="")
            if val_data is not None: print(f"\tavg_val_loss: {val_epoch_loss / total_val_batches}")
    ...
    def evaluate(self, X, y, X_batch_indeces_ext, max_oov):
        ...
        X_mask = tf.expand_dims(tf.not_equal(tf.cast(X, tf.int32), 0), axis=-1)
        y_mask = tf.not_equal(tf.cast(X, tf.int32), 0)
        ...
        for t in range(self.Ty): #decoder loop, iter for each output word
            ...
            if self.use_coverage: # adding coverage loss = lambda * sum(min(alpha, coverage))
                    #MISSING MASK AND AVERAGE PADDING MASK HERE
                coverage_loss = self.lmbda * tf.reduce_sum(tf.math.minimum(alpha_weights, coverage), [0, 1])
                loss += coverage_loss
        ...
        return loss_avg, accuracy_avg, top_k_accuracy_avg
