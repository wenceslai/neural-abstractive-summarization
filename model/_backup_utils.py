...
def save_model(self, save_history=True, save_train_metadata=False, epoch=None, batch_i=None, train_indeces=None, train_time=None, prev_epoch_val_loss=None, save_optimizer=False):
        ...
        if save_train_metadata:
            ...
            'train_time' : train_time, 'prev_epoch_val_loss' : prev_epoch_val_loss}
...
def load_model(self, return_train_metadata=False, load_optimizer=False, load_prev=None):
    ...
    if return_train_metadata: 
        ...
        checkpoint_filename = 'saved_checkpoints' if load_prev is None else "chk_" + load_prev["model_name"] + "_ep" + str(load_prev["epoch"])

        with open(os.path.join(self.save_dir_path, checkpoint_filename + '/train_metadata.pickle'), 'rb') as f:
            
            train_metadata = pickle.load(f)
            print("restoring checkpoint from:", train_metadata['timestamp'], "epoch:", train_metadata['epoch'], "batch_i: ", train_metadata['batch_i'], train_metadata['train_time'])
